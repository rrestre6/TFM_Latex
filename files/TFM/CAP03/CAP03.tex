...
\begin{figure}
	...
	% caption
	\caption[]{Resultados de la simulación del algoritmo de propagación. (a) Amplitud [U.A.] y (b) fase [$-\pi$ a $\pi$ rad] de haz puntual propagado sin atmósfera. (c) Amplitud [U.A.] y (d) fase [$-\pi$ a $\pi$ rad] de haz puntual propagado a través de la atmósfera. El circulo rojo representa la apertura del telescopio}
	\label{fig:propAtmo}
\end{figure} 

La rutina principal realizada en Matlab, que llama las funciones para simular la propagación de la luz a través de la atmósfera, es mostrada en el Apéndice~\ref{anexo:02}. Los parámetros principales en la simulación fueron: longitud de onda ($550$ $nm$), distancia de propagación ($20$ $km$), diámetro del telescopio ($9.25"$), Numero-$f$ ($f/10$) y $C_n^2 \sim 10^{-15}$.
